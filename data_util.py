...
#############################################3
##############################################
def get_vocabulary(vocab_file_path,name_scope="cnn"):

    #save the cache
    cache_vocabulary_pik = 'cache' + '_' + name_scope
    if not os.path.isdir(cache_vocabulary_pik):
        os.makedirs(cache_vocabulary_pik)
    

    #if cache exists, load it
    cache_path = cache_vocabulary_pik + "/" + "vocab.pik"
    print("vocab_cache_path:", cache_path, "file_exists: ",os.path.exists(cache_path))
    if os.path.exists(cache_path):
        ...
        #1. load raw data
        file_object = codecs.open(vocab_file_path,mode="r",encoding="utf-8")
        vocab = file_object.read().splitlines()

        #2. loop
        for i,tuplee in enumerate(vocab):
            word=tuplee            
...
def get_label(training_data_path,name_scope="cnn"):
    #save the cache 
    cache_label_pik = 'cache' + '_' + name_scope
    if not os.path.isdir(cache_label_pik):
        os.makedirs(cache_label_pik)

    cache_path = cache_label_pik + '/' + 'label.pik'
    print('label_cache_path:',cache_path,'file exists:',os.path.exists(cache_path))
    if os.path.exists(cache_path):
        with open(cache_path) as data_f:
            return pickle.load(data_f)
    else:
        label2index = {}
        index2label = {}
        ...
        if not os.path.exists(cache_path):
            with open(cache_path,'ab') as data_f:
                pickle.dump((label2index,index2label),data_f)
